...
export const createExpense = mutation({
  ...
  handler: async (ctx, { userId, amount, purpose, category, date, planId }) => {
    ...
    console.log(`${identity} Created Expense for ${userId} | ${planId}`);
...
export const deleteExpense = mutation({
  ...
  handler: async (ctx, { id }) => {
    const identity = await getIdentityOrThrow(ctx);
    console.log(`${identity} Deleted Expense for expense ID ${id}`);
...
export const updateExpense = mutation({
  ...
  handler: async (ctx, { id, amount, purpose, category, date, userId }) => {
    const identity = await getIdentityOrThrow(ctx);
    console.log(`${identity} patched Expense for expense id ${id}`);
...
export const deleteMultipleExpenses = mutation({
  ...
  handler: async (ctx, { ids }) => {
    const identity = await getIdentityOrThrow(ctx);
    console.log(`${identity} patched Expense for expense id ${ids.toString()}`);
